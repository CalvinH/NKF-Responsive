...
// 2.1 Menu for Mobile Devices
  ...
  $('.top-bar ul').addClass("no-mobile");

  // When the navigation button is clicked, toggle menu
  $(".nav-toggle").click(function() {
    if ( $('.top-bar ul:visible').length < 1 ) {
      $('.top-bar ul').stop(true, true).slideDown(200, function() {
        $('.nav-toggle .arrow').html('-');
        $('.nav-toggle').toggleClass('active');
        $(this).removeClass('no-mobile').removeAttr('style');
      });
    } else {
      $('.top-bar ul').stop(true, true).slideUp(200, function() {
        $('.nav-toggle .arrow').html('+');
        $('.nav-toggle').toggleClass('active');
