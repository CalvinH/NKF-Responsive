...
(function($){
	...
	if(settings.showControls){
		...
		if(settings.centerControls){
			...
			offset = ($container.height() -$control.height());
	...
	if(settings.useCaptions){
		...
		$.each(slides, function(key, value){
			...
			if(alt){
				var $caption = $('<div class="slidey-caption"><h2>' + title + '</h2><p>' + alt + '</p></div>');
				$caption.appendTo($slide);
			}
			else {var $caption = $('<div class="slidey-caption"><h2>' + title + '</h2></div>');
				$caption.appendTo($slide);
				}
		});
